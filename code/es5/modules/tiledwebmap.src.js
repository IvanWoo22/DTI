    ...
    _registerModule(_modules, 'Series/TiledWebMap/TiledWebMapSeries.js', [_modules['Core/Series/SeriesRegistry.js'], _modules['Maps/TilesProviders/TilesProvidersRegistry.js'], _modules['Core/Chart/Chart.js'], _modules['Core/Utilities.js']], function (SeriesRegistry, TilesProvidersRegistry, Chart, U) {
        ...
        var TiledWebMapSeries = /** @class */ (function (_super) {
            ...
            TiledWebMapSeries.prototype.lonLatToTile = function (lonLat, zoom) {
                var lon = lonLat.lon, lat = lonLat.lat, xTile = Math.floor((lon + 180) / 360 * Math.pow(2, zoom)),
            ...
            TiledWebMapSeries.prototype.tileToLonLat = function (xTile, yTile, zTile) {
                var lon = xTile / Math.pow(2, zTile) * 360 - 180,
                    n = Math.PI - 2 * Math.PI * yTile / Math.pow(2, zTile), lat = (180 /
            ...
            TiledWebMapSeries.prototype.drawPoints = function () {
                ...
                var _b = this, tiles = _b.tiles, transformGroups = _b.transformGroups, series = this,
                    options = this.options, provider = options.provider, zoom = mapView.zoom,
                    lambda = pick((mapView.projection.options.rotation &&
                        mapView.projection.options.rotation[0]), 0), worldSize = 400.979322, tileSize = 256,
                    duration = chart.renderer.forExport ? 0 : 200, animateTiles = function (duration) {
                ...
                var zoomFloor = zoom < 0 ? 0 : Math.floor(zoom), maxTile = Math.pow(2, zoomFloor),
                ...
                if (provider && (provider.type || provider.url)) {
                    ...
                    if (mapView.projection && mapView.projection.def) {
                        ...
                        var replaceVariables_1 = function (url, x, y, zoom) {
                            return url
                        ...
                        var addTile = function (x, y, givenZoom, translateX, translateY) {
                            var modX = x % maxTile, modY = y % maxTile, tileX = modX < 0 ? modX + maxTile : modX,
                        ...
                        var startPos = this.lonLatToTile(topLeft, zoomFloor),
                            endPos = this.lonLatToTile(bottomRight, zoomFloor);
                        // calculate group translations based on first loaded tile
                        var firstTileLonLat = this.tileToLonLat(startPos.x, startPos.y, zoomFloor),
                        ...
                        Object.keys(tiles[zoomKey].tiles).forEach(function (key) {
                            if (mapView.projection && mapView.projection.def) {
                                ...
                                var scale_1 = ((tileSize / worldSize) *
                                        ...
                                        Math.pow(2, parseFloat(zoomKey))), scaledTileSize_1 = scale_1 * 256,
                                    firstTile = tiles[zoomKey].tiles[Object.keys(tiles[zoomKey].tiles)[0]],
                                    _a = tiles[zoomKey].tiles[key], posX_1 = _a.posX, posY_1 = _a.posY;
                                if (defined(posX_1) &&
                                    defined(posY_1) &&
                                    defined(firstTile.posX) &&
                                    defined(firstTile.posY)) {
                                    var firstTileLonLat = _this.tileToLonLat(firstTile.posX, firstTile.posY, parseFloat(zoomKey)),
                                        units = mapView.projection.def.forward([
                                            firstTileLonLat.lon + lambda,
                                            firstTileLonLat.lat
                                        ]), firstTilePx = mapView.projectedUnitsToPixels({
                                    ...
                                    if (chart.renderer.globalAnimation &&
                                        chart.hasRendered) {
                                        var startX_1 = Number(tiles[zoomKey].tiles[key].attr('x')),
                                            startY_1 = Number(tiles[zoomKey].tiles[key].attr('y')),
                                            startWidth_1 = Number(tiles[zoomKey].tiles[key].attr('width')),
                                            startHeight_1 = Number(tiles[zoomKey].tiles[key].attr('height'));
            ...
            TiledWebMapSeries.prototype.update = function () {
                var series = this, transformGroups = series.transformGroups, chart = this.chart,
                    mapView = chart.mapView, options = arguments[0], provider = options.provider;
                if (transformGroups) {
        ...
        addEvent(Chart, 'beforeMapViewInit', function (e) {
            var twm = (this.options.series || []).filter(function (s) {
                return s.type === 'tiledwebmap';
            })[0], geoBounds = e.geoBounds;
