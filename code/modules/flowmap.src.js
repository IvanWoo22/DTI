...
(function (factory) {
    ...
    _registerModule(_modules, 'Maps/MapUtilities.js', [], function () {
        ...
        const boundsFromPath = function (path) {
            let x2 = -Number.MAX_VALUE, x1 = Number.MAX_VALUE, y2 = -Number.MAX_VALUE, y1 = Number.MAX_VALUE,
                validBounds;
            path.forEach((seg) => {
    ...
    _registerModule(_modules, 'Maps/Projections/EqualEarth.js', [], function () {
         ...
         *
         * */
        const A1 = 1.340264, A2 = -0.081106, A3 = 0.000893, A4 = 0.003796, M = Math.sqrt(3) / 2.0,
        ...
        class EqualEarth {
            ...
            forward(lonLat) {
                const d = Math.PI / 180, paramLat = Math.asin(M * Math.sin(lonLat[1] * d)),
    ...
    _registerModule(_modules, 'Maps/Projections/Orthographic.js', [], function () {
        ...
        class Orthographic {
            ...
            inverse(xy) {
                const x = xy[0] / scale, y = xy[1] / scale, z = Math.sqrt(x * x + y * y), c = Math.asin(z),
    ...
    _registerModule(_modules, 'Maps/Projection.js', [_modules['Core/Geometry/PolygonClip.js'], _modules['Maps/Projections/ProjectionRegistry.js'], _modules['Core/Utilities.js']], function (PC, registry, U) {
        ...
        class Projection {
             ...
             */
            getRotator(rotation) {
                const deltaLambda = rotation[0] * deg2rad, deltaPhi = (rotation[1] || 0) * deg2rad,
                    deltaGamma = (rotation[2] || 0) * deg2rad;
                const cosDeltaPhi = Math.cos(deltaPhi), sinDeltaPhi = Math.sin(deltaPhi),
                ...
                return {
                    forward: (lonLat) => {
                        ...
                        const lat = lonLat[1] * deg2rad, cosLat = Math.cos(lat), x = Math.cos(lon) * cosLat,
                    ...
                    inverse: (rLonLat) => {
                        ...
                        const lat = rLonLat[1] * deg2rad, cosLat = Math.cos(lat), x = Math.cos(lon) * cosLat,
                            y = Math.sin(lon) * cosLat, sinLat = Math.sin(lat),
                    ...
                    if (isPolygon) {
                        ...
                        if (polarIntersection) {
                            for (let i = 0; i < polygons.length; i++) {
                                const {direction, lat} = polarIntersection, poly = polygons[i],
        ...
        */
        ...
        class MapView {
            ...
            fitToBounds(bounds, padding, redraw = true, animation) {
                ...
                if (b) {
                    const pad = pick(padding, bounds ? 0 : this.options.padding), fullField = this.getField(false),
                    ...
                    if (!this.fitToGeometryCache) {
                        if (fitToGeometry.type === 'MultiPoint') {
                            const positions = fitToGeometry.coordinates
                                    .map((lonLat) => projection.forward(lonLat)), xs = positions.map((pos) => pos[0]),
            ...
            getSVGTransform() {
                const {x, y, width, height} = this.playingField, projectedCenter = this.projection.forward(this.center),
            ...
            lonLatToProjectedUnits(lonLat) {
                ...
                for (const inset of this.insets) {
                    if (inset.options.geoBounds &&
                        ...
                        const insetProjectedPoint = inset.projection.forward([lonLat.lon, lonLat.lat]),
                ...
                for (const inset of this.insets) {
                    if (inset.hitZone &&
                        ...
                        const insetProjectedPoint = inset
                            ...
                            coordinates = inset.projection.inverse([insetProjectedPoint.x, insetProjectedPoint.y]);
            ...
            setView(center, zoom, redraw = true, animation) {
                ...
                if (bounds) {
                    const projectedCenter = this.projection.forward(this.center), {
                        ...
                        width,
                        height
                    } = this.playingField, scale = this.getScale(), bottomLeft = this.projectedUnitsToPixels({
                        x: bounds.x1,
                        y: bounds.y1
            ...
            projectedUnitsToPixels(pos) {
                const scale = this.getScale(), projectedCenter = this.projection.forward(this.center),
            ...
            pixelsToProjectedUnits(pos) {
                const {x, y} = pos, scale = this.getScale(), projectedCenter = this.projection.forward(this.center),
                    field = this.playingField, centerPxX = field.x + field.width / 2,
                    centerPxY = field.y + field.height / 2;
        ...
        class MapViewInset extends MapView {
            ...
            getField(padded = true) {
                ...
                if (hitZone) {
                    const padding = padded ? this.padding : [0, 0, 0, 0], polygon = hitZone.coordinates[0],
                        xs = polygon.map((xy) => xy[0]), ys = polygon.map((xy) => xy[1]),
            ...
            render() {
                ...
                if (borderPath && mapView.group) {
                    ...
                    const crisp = Math.round(this.border.strokeWidth()) % 2 / 2,
                        field = (options.relativeTo === 'mapBoundingBox' &&
        ...
        class FlowMapSeries extends MapLineSeries {
            ...
            static markerEndPath(lCorner, rCorner, topCorner, options) {
                ...
                if (type === 'mushroom') {
                    ...
                    const [xTop, yTop] = topCorner, xMid = (xRight - xLeft) / 2 + xLeft,
