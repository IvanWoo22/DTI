...
(function (factory) {
    ...
    _registerModule(_modules, 'Extensions/Sonification/MIDI.js', [_modules['Extensions/Sonification/SonificationInstrument.js'], _modules['Core/Utilities.js']], function (SonificationInstrument, U) {
        ...
        const {pick} = U;
        const freqToNote = (f) => Math.round(12 * Math.log(f) / Math.LN2 - 48.37632),
            b = (byte, n) => n >>> 8 * byte & 0xFF, getHeader = (nTracks) => [
                0x4D, 0x54, 0x68, 0x64,
                0, 0, 0, 6,
                0, nTracks > 1 ? 1 : 0,
        ...
        function toMIDI(channels) {
            const channelsToAdd = channels.filter((c) => !!c.events.length), numCh = channelsToAdd.length,
                multiCh = numCh > 1;
                    ...
                    mainTracks.forEach((trackOpts) => {
                        ...
                        const addCurrentPointGroup = (groupSpanTime) => {
                            if (pointGroup.length === 1) {
                                ...
                                const points = getGroupedPoints(pointGroupOpts, pointGroup),
